...
function getLunarInfo(date) {
  ...
  const springFestivals = {
    ...
    2022: [2, 1], 2021: [2, 12], 2020: [1, 25], 2019: [2, 5],
    2018: [2, 16], 2017: [1, 28], 2016: [2, 8], 2015: [2, 19],
    2014: [1, 31], 2013: [2, 10], 2012: [1, 23], 2011: [2, 3],
    2010: [2, 14], 2009: [1, 26], 2008: [2, 7], 2007: [2, 18],
  };

  const year = date.getFullYear();
  const sf = springFestivals[year] || [2, 1];
  const sfDate = new Date(year, sf[0] - 1, sf[1]);

  if (date < sfDate) {
    return { year: year - 1, month: Math.floor(Math.random() * 12) + 1, day: Math.floor(Math.random() * 30) + 1 };
  }

  const daysSinceSf = Math.floor((date - sfDate) / (1000 * 60 * 60 * 24));
  const lunarMonthDays = [30, 29, 30, 29, 30, 29, 30, 30, 29, 30, 29, 30]; // 近似大小月

  let remaining = daysSinceSf + 1;
  let month = 1;
...
function getStrokes(name) {
  const basicStrokes = {
    ...
    '明':8,'林':8,'东':8,'欣':8,'青':8,'昊':8,'昌':8,'昂':8,
...
function init() {
  ...
  document.querySelectorAll('.tab-btn').forEach(btn => {
    btn.addEventListener('click', () => {
      ...
      if (page !== state.currentPage && state.currentPage !== 'home') {
